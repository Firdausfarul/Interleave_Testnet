...
async function fetchLiqpool(assetA, assetB) {
  ...
  lp = new LiquidityPoolAsset(
    orderedAsset[0],
    orderedAsset[1],
    LiquidityPoolFeeV18
  );
  poolId = getLiquidityPoolId(
    "constant_product",
...
async function fetchMarket(assetA, assetB) {
  ...
  temp_market = new MarketDetails(assetA, assetB, temp_ob, temp_lp);
...
export async function neptunusCalculate(
  ...
  for (let h = 0; h < loops; h++) {
    //execute pathSend()
    temp_result = pathSend(paths[0], loopsAmount, "execute");
...
async function neptunusExecute(
  sourceAsset,
  destinationAsset,
  address,
  sourceAmount,
