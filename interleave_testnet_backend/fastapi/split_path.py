...
lobstr_server=Server('https://horizon.stellar.lobstr.co')
...
def split_path_main(*, public_key = None, asset_send_code, asset_send_issuer, asset_receive_code,
        ...
        for i in range(len(modified_ob[offer_type])):
            while (amount_sent > 0):
                ...
                if (amount_sent >= depth):
                    ...
                    amount_received = (amount_received + marginal_amount_received)
    ...
    def path_send(path, amount_sent, type):
        ...
        if (type=='execute'):
            result=[]
            result.append(source_amount_sent)
            result.append(received_path)
            result.append(price)
            return result
        if (type=='calc'):
            return price
    ...
    asset_sent=Asset(asset_send_code, asset_send_issuer)
    asset_received=Asset(asset_receive_code, asset_receive_issuer)
    source_asset=asset_sent
    destination_asset=asset_received

    amount_sent=round(float(amount_send), 7)
    # slippage=0.01
    ...
    path=[]
    market=[]
    # print(str(f'{loop_amount:.7f}'))
    if loop_amount == 0:
        return (-69, -69)
    # call_times = 1
    # t = time.time()
    pathresp = lobstr_server.strict_send_paths(
        source_asset=asset_sent,
        source_amount=str(f'{loop_amount:.7f}'),
        destination=[asset_received]
    ).limit(1).call()
    # print(f"Call number-{call_times}: {time.time()-t}")
    # print(pathresp)

    pathtoad=pathresp['_embedded']['records']
    path_limit = 2

    for i in range(min(path_limit, len(pathtoad))):
        path_temp = []
        path_temp.append(asset_sent)
        for j in range(len(pathtoad[i]['path'])):
            if pathtoad[i]['path'][j]['asset_type']=='native':
                path_temp.append(Asset('XLM'))
            else:
    ...
    pathresp = server.strict_send_paths(
        ...
        source_amount=str(f'{amount_sent:.7f}'),
    ...
    total_received = round(total_received, 7)
    print(f"split: {total_received=}, {usual_path_amount=}")
    if operation_detail == "fetch_amount_receive":
        # print(f"{total_received=}, {usual_path_amount=}")
        if total_received < usual_path_amount:
            print("Usual path payment is better")
            return (usual_path_amount, 0)
        return (total_received, total_received/usual_path_amount-1)
    ...
    if total_received < usual_path_amount:
        ...
        Transaction.append_path_payment_strict_send_op(
            ...
            send_amount=str(f'{amount_sent:.7f}'),
            path=path_list
        )
    ...
    Transaction=Transaction.build().to_xdr()
    return (Transaction, total_received, total_received/usual_path_amount-1)